...
export function WhatsAppButton() {
  ...
  const handleClick = () => {
    const text = encodeURIComponent("Hi Choccoladd! I'd like to know more about your products.");
  ...
  return (
    <div className="fixed bottom-6 right-6 z-50 flex flex-col items-end gap-3">
      <AnimatePresence>
        {showTooltip && (
          <motion.div
            ...
            className="bg-white shadow-xl rounded-sm p-4 w-[200px] border border-[#3d1c0a]/10"
          >
            <div className="flex items-center justify-between mb-1.5">
              ...
              <button onClick={() => setShowTooltip(false)} className="text-[#7a5c44] hover:text-[#2c1810]">
                <X size={14} />
              </button>
            </div>
            <p
      ...
      >
        {/* WhatsApp SVG icon */}
        <svg viewBox="0 0 24 24" className="w-7 h-7" fill="#1A0802">
